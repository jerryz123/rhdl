<!-- Defines Ricket's Sv39 MMU ownership, ordering, and first-cut feature contract. -->

# Ricket MMU

`RicketMmu` sits between `RicketCore`'s virtual instruction/data access ports
and the physical L1 cache ports. `RicketTlb` is a reusable fully associative
translation cache, and `RicketPageTableWalker` is a serialized Sv39 walker
...
privilege, `SUM`, and `MXR`. See the parent [Ricket contract](../README.md) for
