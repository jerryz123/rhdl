// Exercises full replacement and same-address FIFO behavior in a depth-one queue.
module queue_one_tb;
  ...
  initial begin
    ...
    assert (ingress_out.ready && egress_out.bits == 8'ha1)
      else $fatal(1, "depth-one queue did not allow replacement");
    tick();
    assert (egress_out.valid && egress_out.bits == 8'hb2)
      else $fatal(1, "same-cycle replacement did not retain the new item");
