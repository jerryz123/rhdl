<!-- Defines the staged architecture, semantics, and validation plan for an optional Rosette formal engine over verified RHDL IR. -->
...
RHDL will treat Rosette as an optional solver-backed consumer of the verified
public core IR. Rosette will not participate in frontend elaboration, determine
generator behavior, add symbolic hardware values to ordinary RHDL programs, or
change the meaning of an existing core operation.

The dependency direction will be:

```text
#lang rhdl or core Builder
            |
            v
...
1. Behavioral equivalence of two RHDL modules under a stated semantic scope.
2. Concrete counterexamples expressed with RHDL port names and types.
...
The proposed implementation lives under `rhdl/formal/`:

| File | Responsibility |
|---|---|
| `main.rhm` | Public Rhombus API and RHDL-native result objects |
...
RHDL objects through their documented Rhombus properties and methods, then
...
The repository boundary checker currently reserves `.rkt` under `rhdl/` for
reader shims. Implementing `engine.rkt` requires a narrow documented exception
for `rhdl/formal/engine.rkt`, plus a rule that `rhdl/formal/` may depend on core
...
RHDL use must not require it. A focused `make formal-test` target will require
...
- paired ports must have mutually equal RHDL hardware types, not merely equal
  packed widths;
...
The interpreter represents each packable RHDL value as one Rosette bitvector
...
- Frontend-defined flat types need no solver special case; their RHDL type
...
Shift interpretation must reproduce RHDL's declared overshift behavior and
...
RHDL's `rtl.dont_care`, uncared decode bits, invalid one-hot selectors, and
...
- Return RHDL-native equivalent, counterexample, unsupported, and unknown
  results.
...
1. Add `rhdl/formal/` to the authoritative package graph as an optional
   consumer of core.
...
4. Permit exactly the planned Rosette interoperability module under
   `rhdl/formal/`; do not allow arbitrary new `.rkt` files throughout `rhdl/`.
...
snapshot instead of corrupting verified RHDL IR.
...
- convert comparison booleans back to one-bit RHDL bitvectors;
- preserve exact fixed-width overflow;
- implement signedness only where the RHDL opcode specifies it; and
...
- `rhdl/formal/main.rhm`
- `tests/formal/api-test.rhm`

The public wrapper owns exact RHDL type checking and converts the plain engine
...
5. Export formal APIs only from `rhdl/formal/main.rhm`; do not add them to
   `#lang rhdl` or `#lang rhdl/base` in milestone 1.
...
- [x] Exact input and output names and RHDL types are checked before solving.
...
Exit criterion: a small bounded synthesis task produces ordinary verified RHDL
...
- `rhdl/README.md` with the formal consumer in the authoritative package graph;
...
- `rhdl/formal/README.md` with the supported semantic matrix and examples.
...
- a multi-clock notion of “cycle” that is not supplied explicitly by the
  caller;
- counterexamples that cannot be mapped back to stable RHDL ports and source
  origins; or
